...
import model.model_asgnet.resnet as models
from model.model_asgnet.module.decoder import build_decoder
from model.model_asgnet.module.ASPP import ASPP
...
class AsgnetModel(nn.Module):
    def __init__(self, args):
        super(AsgnetModel, self).__init__()

        print("MODEL TRAINING="+str(self.training))
        self.training = False
        print("MODEL TRAINING="+str(self.training))
        layers = args.layers
        ...
        #PER ADESSO LO ABBIAMO SEMPRE USATO
        #if self.pyramid:
    ...
    def forward(self, x, s_x=torch.FloatTensor(1,1,3,473,473).cuda(), s_y=torch.FloatTensor(1,1,473,473).cuda(), s_seed=None, y=None):
        ...
        original = query_feat.clone()
        query_feat = self.down_conv(query_feat)
        reduced = query_feat.clone()
